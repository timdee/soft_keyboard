...
public abstract class AnySoftKeyboard extends AnySoftKeyboardKeyboardSwitchedListener implements
    ...
    private volatile List<Touch> successor_touch;
    private volatile List<Touch> successor_touch_s;

    public static Distribution dist;
    public static Distribution distribution;
    public static List<Touch> AddPressure;
...
//        dist = new Distribution(AddSpeed, key.getPrimaryCode());
    ...
    public static void compute() {
        ...
        if (AddPressure.size() % 5 == 0 && AddPressure.size() != 0) {
            if (AddSpeed.size() % 5 == 0 && AddSpeed.size() != 0) {
    ...
    public static void writer() {
        ...
        p_chain.add_touch_list(AnySoftKeyboard.AddPressure);
        s_chain.add_touch_list(AnySoftKeyboard.AddSpeed);

        p_chain.get_distribution();
        p_chain.get_key_distribution();
        p_chain.get_touch_probability((components.Window) p_chain.get_windows(), (Touch) AddPressure);

        s_chain.get_distribution();
        s_chain.get_key_distribution();

        s_chain.get_touch_probability((components.Window) s_chain.get_windows(), (Touch) AddSpeed);

        //      Log.d(TAG, "value %s ",p_chain.get_windows());
